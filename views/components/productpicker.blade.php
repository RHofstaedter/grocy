...
<div class="form-group {{ $additionalGroupCssClasses }}" data-next-input-selector="{{ $nextInputSelector }}" data-disallow-add-product-workflows="{{ boolToString($disallowaddProductWorkflows) }}" data-disallow-all-product-workflows="{{ boolToString($disallowAllProductWorkflows) }}" data-prefill-by-name="{{ $prefillByName }}" data-prefill-by-id="{{ $prefillById }}">
    <label class="w-100" for="product_id">
        ...
        <i class="fa-solid fa-question-circle text-muted" data-toggle="tooltip" data-trigger="hover click" title="{{ $__t('Type a new product name or barcode and hit TAB or ENTER to start a workflow') }}"></i>
        @endif
        @if(!empty($hint))
        <i class="fa-solid fa-question-circle text-muted" data-toggle="tooltip" data-trigger="hover click" title="{{ $hint }}"></i>
        @endif
        <span id="barcode-lookup-disabled-hint" class="small text-muted d-none float-right"> {{ $__t('Barcode lookup is disabled') }}</span>
    ...
    <div id="flow-info-InplaceAddBarcodeToExistingProduct" class="form-text text-info small d-none"><strong><span id="InplaceAddBarcodeToExistingProduct"></span></strong> {{ $__t('will be added to the list of barcodes for the selected product on submit') }}</div>
</div>
