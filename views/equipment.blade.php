...
<div class="row">
    <div class="col-12 col-md-4 pb-3">
        <div class="title-related-links border-bottom mb-2 py-1">
            ...
            <div class="related-links collapse d-md-flex order-2 width-xs-sm-100" id="related-links">
                <a class="btn btn-primary responsive-button m-1 mt-md-0 mb-md-0 float-right" href="{{ $U('/equipment/new') }}">
                    {{ $__t('Add') }}
                </a>
            </div>
        </div>
        ...
        <table id="equipment-table" class="table table-sm table-striped nowrap w-100">
            <thead>
                <tr>
                    <th class="border-right"><a class="text-muted change-table-columns-visibility-button" data-toggle="tooltip" title="{{ $__t('Table options') }}" data-table-selector="#equipment-table" href="#"><i class="fa-solid fa-eye"></i></a>
                    </th>
                    <th>{{ $__t('Name') }}</th>
            ...
            <tbody class="d-none">
                ...
                <tr data-equipment-id="{{ $equipmentItem->id }}">
                    <td class="fit-content border-right">
                        <a class="btn btn-info btn-sm hide-when-embedded hide-on-fullscreen-card" href="{{ $U('/equipment/') }}{{ $equipmentItem->id }}" data-toggle="tooltip" title="{{ $__t('Edit this item') }}">
                        ...
                        <div class="dropdown d-inline-block">
                            ...
                            <div class="table-inline-menu dropdown-menu dropdown-menu-right hide-on-fullscreen-card hide-when-embedded">
                                <a class="dropdown-item equipment-delete-button" type="button" href="#" data-equipment-id="{{ $equipmentItem->id }}" data-equipment-name="{{ $equipmentItem->name }}">
    ...
    <div class="col-12 col-md-8">
        <ul class="nav nav-tabs grocy-tabs mb-1">
            ...
            <li class="nav-item">
                <a class="nav-link" data-toggle="tab" href="#file-userfield-{{$userfield->name}}-tab">{{ $userfield->caption }}</a>
            </li>
            @endif
            @endforeach
        </ul>
        <div class="tab-content grocy-tabs">
            <div class="tab-pane fade show active" id="instruction-manual-tab">
                <div class="card selectedEquipmentInstructionManualCard">
                    <div class="card-header card-header-fullscreen">
                        ...
                        <a class="btn btn-sm btn-outline-secondary py-0 float-right mr-1 selectedEquipmentInstructionManualToggleFullscreenButton" href="#" data-toggle="tooltip" title="{{ $__t('Expand to fullscreen') }}">
                            <i class="fa-solid fa-expand-arrows-alt"></i>
                        </a>
                        <a id="selectedEquipmentInstructionManualDownloadButton" class="btn btn-sm btn-outline-secondary py-0 float-right mr-1" href="#" target="_blank" data-toggle="tooltip" title="{{ $__t('Download file') }}">
            ...
            @foreach($userfields as $userfield)
            @if($userfield->type == \Grocy\Services\UserfieldsService::USERFIELD_TYPE_FILE)
            <div class="tab-pane fade" id="file-userfield-{{$userfield->name}}-tab">
                <div class="card selectedEquipmentInstructionManualCard">
                    <div class="card-header card-header-fullscreen">
                        <span class="selected-equipment-name"></span>
                        <a class="btn btn-sm btn-outline-secondary py-0 float-right mr-1 selectedEquipmentInstructionManualToggleFullscreenButton" href="#" data-toggle="tooltip" title="{{ $__t('Expand to fullscreen') }}">
                        ...
                        <a id="file-userfield-{{$userfield->name}}-download-button" class="btn btn-sm btn-outline-secondary py-0 float-right mr-1" href="#" target="_blank" data-toggle="tooltip" title="{{ $__t('Download file') }}">
                            <i class="fa-solid fa-file-download"></i>
                        </a>
                    </div>
                    <div class="card-body py-0 px-0">
                        <p id="file-userfield-{{$userfield->name}}-empty-hint" class="text-muted font-italic d-none pt-3 pl-3"></p>
                        <embed id="file-userfield-{{$userfield->name}}-embed" class="embed-responsive embed-responsive-4by3" src="" type="application/pdf">
                    </div>
                </div>
            </div>
            @endif
